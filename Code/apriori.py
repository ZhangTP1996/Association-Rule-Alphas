...
binary_data_adjusted_path = './binary_data_adjusted'

if os.path.exists(binary_data_path) is False:
    os.mkdir(binary_data_path)
if os.path.exists(binary_data_adjusted_path) is False:
    os.mkdir(binary_data_adjusted_path)
...
def bin_search(array, num):
    i = bisect_left(array, num)
    if i != len(array) and array[i] == num:
        return i
    return None
...
def process_candidateList(candidateList, ruleList):
    ...
    for rule in candidateList:
        ...
        for element in _subsets:
            if bin_search(ruleList, element):
                continue
            else:
                flag = 0
                break
...
def returnRuleWithMinSupport(binary_data_path, label, ruleList, minSupport, minConfidence, maxLength, file_path, length):
    ...
    for key in binary_data.keys():
        binary_data[key] = binary_data[key].reindex(label.index)
    try:
        f = open(file_path, 'a+')
        returnList = []
...
def runApriori(binary_data, binary_data_path, label, minSupport, minConfidence, elogger):
    ...
    while(len(candidateList) >= 1):
        ...
        if length == 2:
            ...
            for key in binary_data.keys():
                binary_data[key].to_csv('%s/%s.csv' % (binary_data_adjusted_path, str(key)))
            binary_data_path = binary_data_adjusted_path
        length += 1
...
if __name__ == '__main__':
    ...
    for key in binary_data.keys():
        binary_data[key].to_csv('%s/%s.csv' % (binary_data_path, key))
